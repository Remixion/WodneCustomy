...
async function scanGameIdRange(client, puuid, { startId, endId, requestsPerSecond = 2, startFromId, onProgress, shouldStop }) {
  ...
  for (let id = effectiveStart; id <= endId; id++) {
    ...
    if (onProgress && (checked % 5 === 0 || newSinceReport.length)) {
      // await (nie jak w oryginalnej wersji w głównej apce) - tu onProgress
      // sam robi dodatkowe zapytania (pełne dane znalezionego meczu), więc
      // pętla skanowania musi na nie poczekać, żeby nie nakładać requestów.
      await onProgress({ checked, errors, lastId, newlyFound: newSinceReport });
      newSinceReport = [];
    }
