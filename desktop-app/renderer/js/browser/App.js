...
class BrowserApp extends React.Component {
  ...
  async reload() {
    ...
    const allPlayers = await window.api.store.refreshPlayersFromSheets();
    const playersByPuuid = buildPlayersByPuuid(allPlayers);
    const matches = L.buildMatchesFromStore(stored, playersByPuuid, this.state.statics, extractRawStatsMap);
    const agg = L.aggregate(matches);
    this.setState({ matches, agg, playersByPuuid });
  }
  ...
  renderLoading() {
    ...
    return h("div", { style: { minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 22, background: t.bg } },
      ...
      h("div", { style: { color: t.mut, fontFamily: t.disp, letterSpacing: 2, textTransform: "uppercase", fontSize: 12 } }, "Wczytywanie danych")
    );
  }
  ...
  renderSidebar() {
    ...
    return h("header", { style: { position: "sticky", top: 0, zIndex: 5, display: "flex", alignItems: "center", gap: 18, padding: "12px 26px", borderBottom: "1px solid rgba(90,200,255,.18)", background: "rgba(8,10,22,.72)", backdropFilter: "blur(16px)", boxShadow: "0 8px 30px rgba(0,0,0,.35)" } },
      ...
      h("a", { href: "import.html", title: "Import / Zarządzanie", style: { cursor: "pointer", padding: "8px 12px", borderRadius: 9, border: "1px solid " + t.line2, background: "transparent", color: t.mut, fontWeight: 600, fontSize: 12, fontFamily: t.disp } }, "Import / Zarządzanie"),
      h("a", { href: "players.html", title: "Zarządzanie graczami", style: { cursor: "pointer", padding: "8px 12px", borderRadius: 9, border: "1px solid " + t.line2, background: "transparent", color: t.mut, fontWeight: 600, fontSize: 12, fontFamily: t.disp } }, "Gracze"),
